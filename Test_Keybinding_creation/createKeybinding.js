...
function createKeybinding(redirectedKey, destinationText, activeProfileParameter) {
  if (redirectedKey && destinationText && activeProfileParameter) {
    ...
    const keybindingsFilePath = path.join(__dirname, '..', 'keybinding-profiles', `${activeProfileParameter}.json`);
    let keybindings = [];
    if (fs.existsSync(keybindingsFilePath)) {
      keybindings = JSON.parse(fs.readFileSync(keybindingsFilePath, 'utf8'));
    }
    keybindings.push(template);
    fs.writeFileSync(keybindingsFilePath, JSON.stringify(keybindings, null, 2), 'utf8');
